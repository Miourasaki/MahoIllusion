            ...
            for (int i = -radius; i <= radius; i++) {
                for (int j = -radius; j <= radius; j++) {
                    ...
                    if (Math.abs(distance - radius) < 0.5 || distance < radius) {
                        ...
                        Location spawnLocation = player.getLocation().clone().add(0, 1.7 + i, 0).add(vector.multiply(j)); // 在玩家上方1格生成
    ...
    private static void moveArmorStand(ArmorStand armorStand, int maxDistance, Vector direction, Player player) {
        ...
        taskId[0] = scheduler.runTaskTimer(MahoIllusion.getInstance(), () -> {
            ...
            for (int i = 0; i <= 5; i++) {
                ...
                // 生成粒子
                if (i==0)
                    nextLocation.getWorld().spawnParticle(Particle.END_ROD, nextLocation, 10, 0.01, 0.01, 0.01, 0.02);
                // 碰撞检测
